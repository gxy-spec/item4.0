...
- `preview/dynamic_global_view.mp4`：全体参与者的20秒全局动态回放；
...
冻结配置中的完整弓字航线共720 m；按5 m/s飞行需要144 s。CI-E1正式组仅运行20 s，因此UAV实际飞行99.75 m，尚未走完长度约120 m的第一条横向航段，也就没有到达第一个转向点。当前轨迹是对配置航线首段的正确执行，但不能作为“完整弓字航线已验收”的证据。

完整航线现已通过单独的150 s实验完成验收，并增加了10个航点、9个航段和4次折返的硬性检查。结果见 `docs/CI_E1_FULL_ROUTE_RESULTS_20260921.md`。当前20 s结果继续只作为动态传感器同步验收。
